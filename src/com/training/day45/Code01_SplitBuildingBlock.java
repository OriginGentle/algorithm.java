...
public class Code01_SplitBuildingBlock {
    ...
    // 返回值：arr[i....]最少能分几个堆？
    public static int func(int[] arr, int x, int i, int r) {
        if (x == arr.length - 1) {
            return 1;
        }
        // i没到最后一个数
        if (arr[i + 1] - arr[i] <= x) { // 满足条件，贴在一起
            return func(arr, x, i + 1, r);
        } else {
            // 分家
            int p1 = 1 + func(arr, x, i + 1, r);
            // 弥合
            int p2 = Integer.MAX_VALUE;
            int need = (arr[i + 1] - arr[i] - 1) / x;
            if (r >= need) {
                p2 = func(arr, x, i + 1, r - need);
            }
