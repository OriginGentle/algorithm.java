package com.system.III_training.day18;

/**
 * @author ycb
 * @date 2021/9/18-18:42
...
public class Code01_HanoiProblem {
    ...
    public static int step1(int[] arr) {
        int N = arr.length;
        return process1(arr, N - 1, 1, 3, 2);
    }

    // 0...index这些圆盘，arr[0...index] index + 1层塔在哪
    // 在哪?from  去哪?to  另外一个是啥?other
    // arr[0...index]这些状态，是index + 1层汉诺塔问题最优解的第几步
    public static int process1(int[] arr, int index, int from, int to, int other) {
        if (index == -1) {
            return 0;
        }
        if (arr[index] == other) {
        ...
        if (arr[index] == from) {
            return process1(arr, index - 1, from, other, to);
        } else {
            int p1 = (1 << index) - 1;
            int p2 = 1;
            int p3 = process1(arr, index - 1, other, to, from);
            if (p3 == -1) {
                return -1;
            }
            return p1 + p2 + p3;
    ...
    public static int process2(int[] arr, int i, int from, int other, int to) {
        if (i == -1) {
            return 0;
        }
        ...
        if (arr[i] == from) { // 第一大步没走完
            return process2(arr, i - 1, from, to, other);
        } else { // arr[i] == to
            // 已经走完1，2两步了，
            int rest = process2(arr, i - 1, other, from, to); // 第三大步完成的程度
            if (rest == -1) {
                return -1;
            }
            return (1 << i) + rest;
