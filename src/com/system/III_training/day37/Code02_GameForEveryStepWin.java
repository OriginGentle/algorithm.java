...
public class Code02_GameForEveryStepWin {
    ...
    // 当前来到index位置，牌是cands[index]
    // 对手第i轮的得分，scores[index],
    // int hold : i之前保留的牌的总分
    // int cur : 当前轮得到的，之前的牌只算上使用的效果，加成效果是多少
    ...
    public static int process(int[] cards, int[] scores, int index, int hold, int cur, int next) {
        if (index == 25) { // 终止位置，题目隐含条件(牌一共52张)
    ...
    // cur -> 牌点数    ->  * 3 之后是效果
    // next -> 牌点数   ->  * 3之后是效果
    public static int func(int[] cands, int[] scores, int index, int hold, int cur, int next) {
        if (index == 25) { // 最后一张
        ...
        if (all1 > scores[index]) {
            p1 = func(cands, scores, index + 1, hold + cands[index], next, 0);
        }
        // 爆发
        int all2 = hold + cur * 3 + cands[index] * 3;
        int p2 = -1;
        if (all2 > scores[index]) {
            p2 = func(cands, scores, index + 1, hold, next + cands[index], cands[index]);
        }
        return Math.max(p1, p2);
    }
}
