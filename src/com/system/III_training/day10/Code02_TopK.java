 ...
 * @description https://www.lintcode.com/problem/top-k-frequent-words-ii/
 */
public class Code02_TopK {
    ...
    // 比较器
    private NodeHeapComp nodeHeapComp;
    ...
    public void add(String str) {
        ...
        if (preIndex == -1) {
            if (heapSize == heap.length) {
                if (nodeHeapComp.compare(heap[0], curNode) < 0) {
    ...
    private void heapInsert(int index) {
        while (index != 0) {
            ...
            if (nodeHeapComp.compare(heap[index], heap[parent]) < 0) {
                swap(parent, index);
                index = parent;
            } else {
                break;
    ...
    private void heapify(int index, int heapSize) {
        ...
        int smallest = index;
        while (l < heapSize) {
            if (nodeHeapComp.compare(heap[l], heap[index]) < 0) {
                smallest = l;
            }
            if (r < heapSize && nodeHeapComp.compare(heap[r], heap[smallest]) < 0) {
                smallest = r;
            }
            if (smallest != index) {
                swap(smallest, index);
