package com.system.III_training.day10;
...
public class Code05_BooleanEvaluation {
    ...
    public static Info func(char[] str, int L, int R, Info[][] dp) {
        ...
        if (L == R) {
            ...
            for (int split = L + 1; split < R; split += 2) {
                Info left = func(str, L, split - 1, dp);
                Info right = func(str, split + 1, R, dp);
                int a = left.t;
                int b = left.f;
                int c = right.t;
                int d = right.f;
    ...
    public static int process(char[] str, int desired, int L, int R) {
        if (L == R) {
            if (str[L] == '1') {
                return desired;
            } else {
        ...
        if (desired == 1) {
            for (int i = L + 1; i < R; i += 2) {
                switch (str[i]) {
                    case '&':
                        res += process(str, 1, L, i - 1) * process(str, 1, i + 1, R);
                        break;
                    case '|':
                        res += process(str, 1, L, i - 1) * process(str, 0, i + 1, R);
                        res += process(str, 0, L, i - 1) * process(str, 1, i + 1, R);
                        res += process(str, 1, L, i - 1) * process(str, 1, i + 1, R);
                        break;
                    case '^':
                        res += process(str, 1, L, i - 1) * process(str, 0, i + 1, R);
            ...
            for (int i = L + 1; i < R; i += 2) {
                switch (str[i]) {
                    case '&':
                        res += process(str, 0, L, i - 1) * process(str, 1, i + 1, R);
                        res += process(str, 1, L, i - 1) * process(str, 0, i + 1, R);
                        res += process(str, 0, L, i - 1) * process(str, 0, i + 1, R);
                        break;
                    case '|':
                        res += process(str, 0, L, i - 1) * process(str, 0, i + 1, R);
                        break;
    ...
    public static int countEval3(String express, int desired) {
        if (express == null || express.equals("")) {
            return 0;
        }
        char[] exp = express.toCharArray();
