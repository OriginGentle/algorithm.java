package com.system.C_training.day41;
...
//		int p0 = func(arr, i + 1);
//		// p1使用for循环搞定的！
//		int p1 = 找到arr[i..s]是最短的，且能搞出AABB来的(4个) + func(arr, s + 1);
//		// p2使用for循环搞定的！
//		int p2 = 找到arr[i..t]是最短的，且能搞出ABAB来的(4个) + func(arr, t + 1);
//		// p3使用for循环搞定的！
//		int p3 = 找到arr[i..k]是最短的，且能搞出ABBA来的(4个) + func(arr, k + 1);
//		// p4没用
//		int p4 = 找到arr[i..f]是最短的，且能搞出AAAA来的(4个) + func(arr, f + 1);
//		return p0~p4的最大值
//	}
    ...
    public static int process2(int[] var, int[][] imap, int i) {
        if (i + 4 > var.length) {
            return 0;
        }
        int p0 = process2(var, imap, i + 1);
        // AABB
        int p1 = 0;
        int rightClosedP1A2 = rightClosed(imap, var[i], i);
        if (rightClosedP1A2 != -1) {
            for (int next = rightClosedP1A2 + 1; next < var.length; next++) {
                if (var[i] != var[next]) {
                    int rightClosedP1B2 = rightClosed(imap, var[next], next);
                    if (rightClosedP1B2 != -1) {
                        p1 = Math.max(p1, 4 + process2(var, imap, rightClosedP1B2 + 1));
                    }
                }
            }
        }
        ...
        // ABAB
        int p2 = 0;
        for (int p2B1 = i + 1; p2B1 < var.length; p2B1++) {
            if (var[i] != var[p2B1]) {
                int rightClosedP2A2 = rightClosed(imap, var[i], p2B1);
                if (rightClosedP2A2 != -1) {
                    int rightClosedP2B2 = rightClosed(imap, var[p2B1], rightClosedP2A2);
                    if (rightClosedP2B2 != -1) {
                        p2 = Math.max(p2, 4 + process2(var, imap, rightClosedP2B2 + 1));
                    }
                }
            }
        }
        ...
        // ABBA
        int p3 = 0;
        for (int p3B1 = i + 1; p3B1 < var.length; p3B1++) {
            if (var[i] != var[p3B1]) {
                int rightClosedP3B2 = rightClosed(imap, var[p3B1], p3B1);
                if (rightClosedP3B2 != -1) {
                    int rightClosedP3A2 = rightClosed(imap, var[i], rightClosedP3B2);
                    if (rightClosedP3A2 != -1) {
                        p3 = Math.max(p3, 4 + process2(var, imap, rightClosedP3A2 + 1));
                    }
                }
            }
        }
        // AAAA
        int p4 = 0;
        int rightClosedP4A2 = rightClosed(imap, var[i], i);
        int rightClosedP4A3 = rightClosedP4A2 == -1 ? -1 : rightClosed(imap, var[i], rightClosedP4A2);
        int rightClosedP4A4 = rightClosedP4A3 == -1 ? -1 : rightClosed(imap, var[i], rightClosedP4A3);
        if (rightClosedP4A4 != -1) {
            p4 = Math.max(p4, 4 + process2(var, imap, rightClosedP4A4 + 1));
        }
