package com.system.IV_weekly.code_2022_05_2_week;
...
public class Code01_TwoObjectMaxValue {
    ...
    public static int max1(int[] w, int[] v, int bag) {
        return process(w, v, 0, 2, bag);
    }

    public static int process(int[] w, int[] v, int index, int restNum, int restWeight) {
        if (restNum < 0 || restWeight < 0) {
            return -1;
        }
        if (index == w.length) {
            return 0;
        }
        int p1 = process(w, v, index + 1, restNum, restWeight);
        int p2 = -1;
        int next = process(w, v, index + 1, restNum - 1, restWeight - w[index]);
        if (next != -1) {
            p2 = v[index] + next;
        }
