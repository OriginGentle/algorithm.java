package com.system.IV_weekly.code_2022_09_4_week;

/**
 * @author ycb
 * @date 2022/10/13-10:37
 * @desc 智能机器人要坐专用电梯把货物送到指定地点
...
public class Code01_RobotDeliverGoods {
    ...
    public static int minCost1(int k, int[] from, int[] to) {
        ...
        return process1(0, k, from, to);
    }

    public static int process1(int i, int k, int[] from, int[] to) {
        if (i == k) {
            int ans = 0, cur = 1;
            for (int j = 0; j < k; j++) {
            ...
            for (int j = i; j < k; j++) {
                swap(from, to, i, j);
                ans = Math.min(ans, process1(i + 1, k, from, to));
                swap(from, to, i, j);
            }
            return ans;
        }
    ...
    public static int minCost2(int k, int[] from, int[] to) {
        ...
        return process2(0, 0, k, from, to, dp);
    }

    public static int process2(int status, int i, int k, int[] from, int[] to, int[][] dp) {
        if (dp[status][i] != -1)
            return dp[status][i];
        ...
        if (status == (1 << k) - 1) {
            ...
            for (int j = 0; j < k; j++) {
                if ((status & (1 << j)) == 0) {
                    ...
                    int next = process2(status | (1 << j), j, k, from, to, dp);
    ...
    // 最后回到第一层，返回最小耗时
    public static int func(int status, int last, int k, int[] from, int[] to) {
        if (status == (1 << k) - 1) { // 所有货送完了，回到1层去了
            ...
            for (int cur = 0; cur < k; cur++) {
                ...
                if ((status & (1 << cur)) == 0) { // 当前cur号的货物，可以去尝试
                    ...
                    int deliver = Math.abs(to[cur] - from[cur]);

                    int next = func(status | (1 << cur), cur, k, from, to);
