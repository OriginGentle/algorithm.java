package com.system.IV_weekly.code_2023_01_2_week;
 ...
 * @desc 给定一个正整数 x，我们将会写出一个形如 x (op1) x (op2) x (op3) x ... 的表达式
...
public class Code03_LeastOperatorsToExpressNumber {
    ...
    public static int leastOpsExpressTarget(int x, int target) {
        return process(0, target, x, new HashMap<>()) - 1;
    }

    private static int process(int i, int target, int x,
        ...
        if (target > 0 && i < 39) {
            if (target == 1) {
                ...
                int div = target / x;
                int mod = target % x;
                int p1 = mod * cost(i) + process(i + 1, div, x, dp);
                int p2 = (x - mod) * cost(i) + process(i + 1, div + 1, x, dp);
                ans = Math.min(p1, p2);
            }
        }
        if (!dp.containsKey(i)) {
