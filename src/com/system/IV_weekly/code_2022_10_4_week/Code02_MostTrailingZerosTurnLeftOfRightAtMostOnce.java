...
public class Code02_MostTrailingZerosTurnLeftOfRightAtMostOnce {
    ...
    public static int mostTrailingZeros(int[][] matrix) {
        ...
        // f2[i][j] : matrix[i][j]自己有多少个2的因子
        int[][] f2 = new int[n][m];
        // f5[i][j] : matrix[i][j]自己有多少个5的因子
        int[][] f5 = new int[n][m];
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < m; j++) {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < m; j++) {
                ...
                ans = Math.max(ans, Math.max(Math.max(p1, p2),
                        Math.max(Math.max(p3, p4), Math.max(p5, p6))));
            }
        }
        return ans;
    }
