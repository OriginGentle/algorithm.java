package com.system.IV_weekly.code_2022_10_4_week;
...
public class Code07_RemoveMostKContinuousSameLongest {
    ...
    public static int longest1(int[] arr, int k) {
        return process(arr, 0, new int[arr.length], 0, k);
    }

    public static int process(int[] arr, int i, int[] path, int size, int k) {
        if (k < 0) {
            return 0;
        }
        if (i == arr.length) {
            ...
            path[size] = arr[i];
            int p1 = process(arr, i + 1, path, size + 1, k);
            int p2 = process(arr, i + 1, path, size, k - 1);
    ...
    public static int longest2(int[] arr, int k) {
        HashMap<Integer, LinkedList<Integer>> valueIndex = new HashMap<>();
        int ans = 1;
        for (int i = 0; i < arr.length; i++) {
            int value = arr[i];
            if (!valueIndex.containsKey(value))
                valueIndex.put(value, new LinkedList<>());

            LinkedList<Integer> indies = valueIndex.get(value);
