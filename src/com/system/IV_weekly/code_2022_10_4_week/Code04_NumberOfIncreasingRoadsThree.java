...
public class Code04_NumberOfIncreasingRoadsThree {
    ...
    public static int num1(int[][] roads) {
        ...
        return process(roads, 0, 3, 0, dp);
    }

    public static int process(int[][] roads, int i, int rest, int end, int[][][] dp) {
        if (rest == 0) {
            return 1;
        }
        if (i == roads.length) {
        ...
        int p1 = process(roads, i + 1, rest, end, dp);
        int p2 = roads[i][0] > end ? process(roads, i + 1, rest - 1, roads[i][1], dp) : 0;
        int ans = p1 + p2;
        dp[i][rest][end] = ans;
        return ans;
    }
