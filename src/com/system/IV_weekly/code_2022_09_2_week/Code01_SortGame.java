package com.system.IV_weekly.code_2022_09_2_week;
...
public class Code01_SortGame {
    ...
    public static int[] game1(int[] arr, int[][] op) {
        ...
        for (int[] p : op) {
            if (p[1] == 0)
                Arrays.sort(help, 0, p[0], Comparator.comparingInt(a -> a));
            else
                Arrays.sort(help, 0, p[0], (a, b) -> b - a);
        }
    ...
    public static int[] game2(int[] arr, int[][] op) {
        ...
        for (; ai >= op[stack[l]][0]; ai--)
            ans[ai] = arr[ai];

        TreeSet<Number> set = new TreeSet<>(new NumberComp());
        ...
        while (l != r) {
            ...
            if (l != r) {
                ...
                if (cur[1] == 0) {
                    for (int i = 0; i < num; i++)
                        ans[ai--] = set.pollLast().val;
                } else {
                    for (int i = 0; i < num; i++) {
                        ans[ai--] = set.pollFirst().val;
                    }
                }

            } else {
                while (!set.isEmpty())
                    ans[ai--] = cur[1] == 0 ? set.pollLast().val : set.pollFirst().val;
    ...
    public static class NumberComp implements Comparator<Number> {
        ...
        public int compare(Number o1, Number o2) {
            return o1.val != o2.val ? o1.val - o2.val : o1.idx - o2.idx;
        }
    }
