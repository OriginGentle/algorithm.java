...
public class Code06_TopMinSubsquenceSum {
    ...
    private static void process(int[] arr, int index, int sum, ArrayList<Integer> allAns) {
        if (index == arr.length) {
            allAns.add(sum);
        } else {
            process(arr, index + 1, sum, allAns);
            process(arr, index + 1, sum + arr[index], allAns);
        }
    }
