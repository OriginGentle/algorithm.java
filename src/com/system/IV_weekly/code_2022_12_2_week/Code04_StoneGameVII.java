package com.system.IV_weekly.code_2022_12_2_week;

import java.util.Arrays;

/**
 * @author ycb
 * @date 2022/12/17-00:36
 * @desc 石子游戏中，爱丽丝和鲍勃轮流进行自己的回合，爱丽丝先开始 。
 * 有 n 块石子排成一排。
...
public class Code04_StoneGameVII {
    ...
    public static int s(int[] stones, int sum, int L, int R) {
        ...
        int p1 = f(stones, sum - stones[L], L + 1, R);

        int against2 = sum - stones[R] + s(stones, sum - stones[R], L, R - 1);
        int p2 = f(stones, sum - stones[R], L, R - 1);

        return against1 - p1 > against2 - p2 ? p1 : p2;
    }
