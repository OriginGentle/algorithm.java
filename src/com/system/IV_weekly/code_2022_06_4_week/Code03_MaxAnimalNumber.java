package com.system.IV_weekly.code_2022_06_4_week;

/**
 * @author ycb
 * @date 2022/7/1-13:36
 * @desc 有n个动物重量分别是a1、a2、a3.....an，
 * 这群动物一起玩叠罗汉游戏，
...
public class Code03_MaxAnimalNumber {
    ...
    public static int process(int[] arr, int index, int pre, int[][] dp) {
        if (index == arr.length) {
            return 0;
        }
        if (dp[index][pre] != -1) {
        ...
        int p1 = process(arr, index + 1, pre, dp);
        int p2 = 0;
        if (arr[index] >= pre) {
            p2 = 1 + process(arr, index + 1, pre + arr[index], dp);
        }
