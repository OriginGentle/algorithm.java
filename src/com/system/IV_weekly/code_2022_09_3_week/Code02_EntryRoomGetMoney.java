package com.system.IV_weekly.code_2022_09_3_week;
...
public class Code02_EntryRoomGetMoney {
    ...
    public static int maxMoney1(int n, int[] p, int[] w) {
        ...
        Arrays.fill(dp, -1);
        for (int i = 0; i < n; i++) {
            ans = Math.max(ans, process(i, rooms, n, dp));
        }
        return ans;
    }

    public static int process(int index, int[][] rooms, int n, int[] dp) {
        if (dp[index] != -1) {
            return dp[index];
        }
        int next = 0;
        for (int i = index + 1; i < n; i++) {
            if (reach(rooms[index][0], rooms[i][0])) {
                next = Math.max(next, process(i, rooms, n, dp));
            }
        }
