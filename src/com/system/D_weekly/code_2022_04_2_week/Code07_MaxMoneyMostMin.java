package com.system.D_weekly.code_2022_04_2_week;
...
public class Code07_MaxMoneyMostMin {
    ...
    public static long mostMin1(int a, int b, int[] value, int[] trouble) {
        return process(a, value, trouble, 0);
    }

    public static long process(int boss, int[] value, int[] trouble, int index) {
        if (index == value.length) {
            ...
            for (int i = index; i < value.length; i++) {
                ...
                ans = Math.min(ans, process(boss, value, trouble, index + 1));
                swap(value, trouble, i, index);
            }
            return ans;
        }
