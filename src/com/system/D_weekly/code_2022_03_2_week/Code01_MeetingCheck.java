...
public class Code01_MeetingCheck {
    ...
    public static boolean[] reserveMeetings(int m, int[][] meetings) {
        ...
        for (int[] meeting : reMeetings) {
            if (segmentTree.queryMax(meeting[2], meeting[3]) < m) {
                ans[meeting[0]] = true;
                segmentTree.add(meeting[2], meeting[3], 1);
            }
        }
        return ans;
    }

    private static int rank(int[] ranks, int num) {
        int l = 0;
        int r = ranks.length - 1;
        int m = 0;
        int ans = 0;
        while (l <= r) {
            m = (l + r) / 2;
            ...
            for (int j = 0; j < testTimes2; j++) {
                ...
                if (Math.random() < 0.5) {
                    ...
                    if (st.queryMax(L, R) != right.queryMax(L, R)) {
                        System.out.println(st.queryMax(L, R));
                        System.out.println(right.queryMax(L, R));
