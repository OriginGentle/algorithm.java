package com.system.D_weekly.code_2022_08_5_week;
 ...
 * 0 <= arr[i] <= 10^6
 */
public class Code03_SubarrayMakeSortedMaxSum {
    ...
    public static long maxSum1(int[] arr) {
        ...
        for (int i = 0; i < n; i++) {
            ans = Math.max(ans, process(arr, i, arr[i], dp));
        }
        return ans;
    }

    public static long process(int[] arr, int i, int p, long[][] dp) {
        if (p <= 0 || i == -1) {
            return 0;
        }
        if (dp[i][p] != -1) {
        ...
        long next = process(arr, i - 1, cur - 1, dp);
