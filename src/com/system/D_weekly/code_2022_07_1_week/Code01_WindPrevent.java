package com.system.D_weekly.code_2022_07_1_week;

/**
 * @author ycb
 * @date 2022/7/10-16:07
 * @desc 给定一个二维数组matrix，数组中的每个元素代表一棵树的高度。
...
public class Code01_WindPrevent {
    ...
    public static int bestHeight2(int[][] matrix, int k) {
        int n = matrix.length;
        int m = matrix[0].length;
        int[][] windowMax = new int[m][n];
        int[][] windowLR = new int[m][2];
        for (int i = 0; i < k; i++) {
            addRow(matrix, m, i, windowMax, windowLR);
        }
        int ans = bottleNeck(matrix, m, windowMax, windowLR);
        for (int i = k; i < n; i++) {
            addRow(matrix, m, i, windowMax, windowLR);
            deleteRow(m, i - k, windowMax, windowLR);
            ans = Math.max(ans, bottleNeck(matrix, m, windowMax, windowLR));
        }
        return ans;
    }

    public static void addRow(int[][] matrix, int m, int row, int[][] windowMax, int[][] windowLR) {
        for (int col = 0; col < m; col++) {
            while (windowLR[col][0] != windowLR[col][1] &&
                    matrix[windowMax[col][windowLR[col][1] - 1]][col] <= matrix[row][col]) {
                windowLR[col][1]--;
            }
            windowMax[col][windowLR[col][1]++] = row;
        }
    }

    public static void deleteRow(int m, int row, int[][] windowMax, int[][] windowLR) {
        for (int col = 0; col < m; col++) {
            if (windowMax[col][windowLR[col][0]] == row) {
                windowLR[col][0]++;
            }
        }
    }

    public static int bottleNeck(int[][] matrix, int m, int[][] windowMax, int[][] windowLR) {
        int ans = Integer.MAX_VALUE;
        for (int col = 0; col < m; col++) {
            ans = Math.min(ans, matrix[windowMax[col][windowLR[col][0]]][col]);
        }
        return ans;
    }
