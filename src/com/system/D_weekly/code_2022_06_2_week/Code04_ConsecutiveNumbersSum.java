package com.system.D_weekly.code_2022_06_2_week;

/**
 * @author ycb
 * @date 2022/6/19-14:51
 * @desc https://leetcode.com/problems/consecutive-numbers-sum/
 */
public class Code04_ConsecutiveNumbersSum {

    // 如果存在答案，则满足式子： N = (x + 1) + (x + 2) + (x + 3) + ... + (x + k)
    // 化简可得：N = kx + k(k + 1) / 2
    // 左右两边乘以2 ： 2N = 2kx + k(k + 1) --> 2N = k(2x + k + 1)
    // k : 项数   x : 起始数
    // 也是说，只要给定一组不同的 k 和 x，就对应的一种方案
    // k为奇数，那么2x + k + 1就是偶数
    // k为偶数，那么2x + k + 1就是奇数
    // 2N 奇数因子K, 2x + k + 1
    ...
    public static void main(String[] args) {
        int N = 3 * 5 * 7;
        int ans = consecutiveNumbersSum2(N);
