...
public class Code05_NetworkDelayGuess {
    ...
    public static int ways(int[] times) {
        ...
        process(times, 0, status, set);
        return set.size();
    }

    private static void process(int[] times, int i, int[] status, HashSet<String> ans) {
        if (i == times.length) {

            int a = 0, b = 0, c = 0;
            int cnt = count(status);
    ...
    private static int count(int[] status) {
        int ans = 0;
        for (int i = 0; i < 3; i++) {
            if (status[i] != 0) {
                ans++;
