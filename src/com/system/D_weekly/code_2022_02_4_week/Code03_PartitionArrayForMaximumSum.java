package com.system.D_weekly.code_2022_02_4_week;
 ...
 * @description https://leetcode.com/problems/partition-array-for-maximum-sum/
 */
public class Code03_PartitionArrayForMaximumSum {
    ...
    public int maxSumAfterPartitioning1(int[] arr, int k) {
        ...
        Arrays.fill(dp, -1);
        return process(arr, k, arr.length - 1, dp);
    }
    ...
    private static int process(int[] arr, int k, int index, int[] dp) {
        if (index == -1) {
            return 0;
        }
        if (dp[index] != -1) {
        ...
        for (int i = index, j = 1; i >= 0 && j <= k; i--, j++) {
            ...
            ans = Math.max(ans, process(arr, k, i - 1, dp) + (index - i + 1) * max);
        }
        dp[index] = ans;
        return ans;
    }
