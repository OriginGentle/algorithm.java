 ...
 * @desc 给你一个 n 个点的带权无向连通图，节点编号为 0 到 n-1
 ...
 * 技巧：并查集、最小生成树、求联通图中的桥，并查集、最小生成树
 */
public class Code05_FindCriticalAndPseudoCriticalEdges {
    ...
    // 并查集初始化
    public static void buildUnionSet(int n) {
        for (int i = 0; i < n; i++) {
            father[i] = i;
            size[i] = 1;
        }
    ...
    public static void buildEdges(int[][] e) {
        ...
        Arrays.sort(edges, 0, m, Comparator.comparingInt(a -> a[3]));
    }
