...
public class Problem_0812_LargestTriangleArea {
    ...
    public static double largestTriangleArea1(int[][] points) {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = i + 1; j < n; j++) {
                for (int k = j + 1; k < n; k++) {
                    ans = Math.max(ans,
                            cross(points[i][0], points[i][1], points[j][0], points[j][1], points[k][0], points[k][1]));
                }
            }
        }
        return ans;
    ...
    public static double cross(int x1, int y1, int x2, int y2, int x3, int y3) {
