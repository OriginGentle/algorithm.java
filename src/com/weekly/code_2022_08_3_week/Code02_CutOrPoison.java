...
public class Code02_CutOrPoison {
    ...
    public static int process(int[] cuts, int[] poisons, int index, int restHp, int poisonEffect, int[][][] dp) {
        restHp -= poisonEffect;
        if (restHp <= 0) {
            return index + 1;
        }
        ...
        int p1 = restHp <= cuts[index] ? (index + 1)
                : process(cuts, poisons, index + 1, restHp - cuts[index], poisonEffect, dp);
        int p2 = process(cuts, poisons, index + 1, restHp, poisonEffect + poisons[index], dp);
    ...
    public static boolean ok(int[] cuts, int[] poisons, int hp, int limit) {
        ...
        for (int i = 0, j = 1; i < n; i++, j++) {
            hp -= Math.max(cuts[i], (long) (limit - j) * (long) poisons[i]);
            if (hp <= 0) {
                return true;
            }
        }
