package com.weekly.code_2022_06_1_week;

import com.basic.day47.Code03_DinicAlgorithm;
 ...
 * @desc 给你一个字符串化学式 formula ，返回 每种原子的数量 。
...
public class Code03_NumberOfAtoms {
    ...
    public static String countOfAtoms(String s) {
        char[] str = s.toCharArray();
        Info info = process(str, 0);
        StringBuilder sb = new StringBuilder();
        for (String key : info.cntMap.keySet()) {
            sb.append(key);
            Integer cnt = info.cntMap.get(key);
            if (cnt > 1) {
                sb.append(cnt);
            }
        }
            ...
            if ((s[i] >= 'A' && s[i] <= 'Z') || s[i] == '(') { // 大写字母 或 '('
                ...
                if (builder.length() != 0 || info != null) {
                    ...
                    if (builder.length() != 0) {
                        ...
                        for (String key : info.cntMap.keySet()) {
                            cntMap.put(key,
                                    cntMap.getOrDefault(key, 0) + info.cntMap.get(key) * cnt);
                        }
                        info = null;
                    }
                    cnt = 0;
